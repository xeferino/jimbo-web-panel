        ...
        foreach ($raffles as $key => $value) {
            ...
            if($favorite) {
                ...
                array_push($data, [
                    ...
                    'cash_to_draw'      => $value->type == 'raffle' ? Helper::amountJib($value->cash_to_draw) : '',
            ...
            foreach ($raffles as $key => $value) {
                if($value->days_ago >= 0) {
                    ...
                    array_push($data, [
                        ...
                        'cash_to_draw'      => $value->type == 'raffle' ? Helper::amountJib($value->cash_to_draw) : $value->title,
                        'date_start'        => $value->date_start->format('d/m/Y'),
                        'date_end'          => $value->date_end->format('d/m/Y'),
                        'date_release'      => $value->date_release->format('d/m/Y'),
        ...
        try {
            ...
            return response()->json([
                'raffle' => [
                    ...
                    'cash_to_draw'  => $raffle->type == 'raffle' ? Helper::amountJib($raffle->cash_to_draw) : '',
    ...
    public function show($id)
    {
        try {
            ...
            foreach ($raffle->tickets as $key => $value) {
                ...
                array_push($tickets, [
                    ...
                    'promotions'    => [
                        ...
                        'price'     => Helper::amountJib($value->promotion->price),
                        'price_jib' => Helper::amountJib($value->promotion->price).' ('.$value->promotion->quantity.' BOLETOS)',
            ...
            return response()->json([
                'raflle' => [
                    'id'                => $raffle->id,
                    'title'             => $raffle->type == 'raffle' ? Helper::amountJib($raffle->cash_to_draw) : '',
                    'description'       => $raffle->description,
                    'brand'             => $raffle->brand,
                    'promoter'          => $raffle->promoter,
                    'place'             => $raffle->place,
                    'provider'          => $raffle->provider,
                    'cash_to_draw'      => $raffle->type == 'raffle' ? Helper::amountJib($raffle->cash_to_draw) : $raffle->title,
                    //'cash_to_draw'      => $raffle->type == 'raffle' ? Helper::amount($raffle->cash_to_draw) : $raffle->title,
                    'logo'              => $raffle->image != 'raffle.jpg' ? $this->asset.$raffle->image : $this->asset.'raffle.jpg',
                    'date_start'        => $raffle->date_start->format('d/m/Y'),
                    'date_end'          => $raffle->date_end->format('d/m/Y'),
                    ...
                    'remaining_days'    => $days,
                    'awards' => [
                        'first'     => Helper::amountJib(($raffle->cash_to_draw*$raffle->prize_1)/100),
                        'second'    => Helper::amountJib(($raffle->cash_to_draw*$raffle->prize_2)/100),
                        'third'     => Helper::amountJib(($raffle->cash_to_draw*$raffle->prize_3)/100),
                        'fourth'    => Helper::amountJib(($raffle->cash_to_draw*$raffle->prize_4)/100),
                        'fifth'     => Helper::amountJib(($raffle->cash_to_draw*$raffle->prize_5)/100),
                        'sixth'     => Helper::amountJib(($raffle->cash_to_draw*$raffle->prize_6)/100),
                        'seventh'   => Helper::amountJib(($raffle->cash_to_draw*$raffle->prize_7)/100),
                        'eighth'    => Helper::amountJib(($raffle->cash_to_draw*$raffle->prize_8)/100),
                        'nineth'    => Helper::amountJib(($raffle->cash_to_draw*$raffle->prize_9)/100),
                        'tenth'     => Helper::amountJib(($raffle->cash_to_draw*$raffle->prize_10)/100),
                    ],
                    'tickets' => $tickets,
                ]
            ], 200);
        ...
        try {
            ...
            if($ticket) {
                ...
                array_push($data, [
                    'raflle' => [
                        'id'            => $ticket->raffle->id,
                        'title'         => $ticket->raffle->type == 'raffle' ? Helper::amountJib($ticket->raffle->cash_to_draw) : '',
                        ...
                        'cash_to_draw'  => $ticket->raffle->type == 'raffle' ? Helper::amountJib($ticket->raffle->cash_to_draw) : $ticket->raffle->title,
                        ...
                        'promotion' => [
                            ...
                            'price' => Helper::amountJib($ticket->promotion->price).' ('.$ticket->promotion->quantity.' BOLETOS)',
            ...
            return response()->json([
                'raflle' => [
                    ...
                    'provider'      => $ticket->raffle->provider,
                    //'cash_to_draw'  => Helper::amount($ticket->raffle->cash_to_draw),
                    'cash_to_draw'  => Helper::amountJib($ticket->raffle->cash_to_draw),
                    'logo'          => $ticket->raffle->image != 'raffle.jpg' ? $this->asset.$ticket->raffle->image : $this->asset.'raffle.jpg',
                    'date_start'    => $ticket->raffle->date_start->format('d/m/Y'),
                    'date_end'      => $ticket->raffle->date_end->format('d/m/Y'),
                    'date_release'  => $ticket->raffle->date_release->format('d/m/Y'),
